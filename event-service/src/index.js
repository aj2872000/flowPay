...
const { startWebhookRetryJob, stopWebhookRetryJob } = require("./jobs/webhookRetryJob");

const start = async () => {
  await connectDB();

  const server = app.listen(config.port, () => {
    logger.info("✅ Event Service running", {
      port:    config.port,
      env:     config.nodeEnv,
  ...
  // Start the webhook delivery retry cron
  startWebhookRetryJob();

  const shutdown = (signal) => {
    logger.info(`${signal} – shutting down event service`);
    stopWebhookRetryJob();
    server.close(async () => {
